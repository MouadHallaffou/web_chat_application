...
class MessageService {
    // Envoyer un message
    static async sendMessage(senderId, receiverId, content, type = 'text', metadata) {
        ...
        const friendship = await models_1.Friendship.findFriendship(senderId, receiverId);
        const areFriends = friendship && friendship.status === 'active';
        // Créer ou trouver la conversation
        let conversation = await models_1.Conversation.findOrCreateDirectConversation(senderId, receiverId);
        // Créer le message
    ...
    static async getConversationMessages(conversationId, userId, options = {}) {
        ...
        await models_1.Message.markConversationAsRead(conversationId, userId);
        await conversation.markAsRead(userId);
        // Obtenir les messages
        const messages = await models_1.Message.getConversationMessages(conversationId, options);
        return messages;
    }
    // Obtenir les conversations d'un utilisateur
    static async getUserConversations(userId) {
        const conversations = await models_1.Conversation.getUserConversations(userId);
    ...
    static async getMessageStats(userId) {
        ...
        const [totalMessages, unreadMessages, todayMessages] = await Promise.all([
            models_1.Message.countDocuments({ conversationId: { $in: conversationIds } }),
